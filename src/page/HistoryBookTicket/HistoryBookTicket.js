...
import { deleteTicketUserService, getAllTicketUserService, updateTicketUserService } from '../../service/TicketService'
...
const HistoryBookTicket = (props) => {
  ...
  const [sdkReady, setSdkReady] = useState(false)
    ...
    const addPaypalScript = async () => {
        ...
        script.src = `https://www.paypal.com/sdk/js?client-id=AX7pxzmolO22Q04Hw2cG8CWBRR83w5fAlCzZ7e1ElmeK5xVPXombTGJN9WuLW5_w9MO26pZFzbLiY_CA`
        script.async = true
        script.onload = () => {
            setSdkReady(true)
        }
    ...
    const onSuccesPaypal = async (details, data) => {
        let ticket = {
                              ...
                              {data?.map((item, index) => {
                                  return (
                                      <tr key={item?._id}>
                                          ...
                                          <td>{item?.trip?.departure?.value} - {item?.trip?.destination?.value}</td>
            ...
            <Modal.Body>
                <div className="row">
                   <div className='col-12 content-modal'>
                        ...
                        <div className='item'>
                            <span>Khởi hành:</span>
                            <span className='item'>{ticketUpdate?.trip?.departure?.value} - {ticketUpdate?.trip?.destination?.value}</span>
                        </div>
                        <div className='item'>
                            <span>Ngày:</span>
                            <span className='item'>{ticketUpdate?.trip?.dayStart} - {ticketUpdate?.trip?.time?.value}</span>
                        </div>
                        <div className='number'>
                            <span onClick={() => numberBookUpdate === 0 ? numberBookUpdate :setNumberBookUpdate(numberBookUpdate - 1)}>-</span>
        ...
        <Modal show={showPayment} onHide={handlePaymentClose} >
            ...
            <Modal.Body>
                <div className="row modal-payment">
                    ...
                    {valuePayment === '653cf2766134d7fbf103d53e'  && sdkReady ?
